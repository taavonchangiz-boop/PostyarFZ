    ...
    for (var i = 0; i < parts.length; i++) {
        ...
        if (i === 0) {
            ...
            if (adminMatch) {
                ...
                bubble.innerHTML = '<div style="font-size:0.75rem; color:#E5B44E; font-weight:bold; margin-bottom:0.4rem;">👤 پیام کاربر (' + (t.user_name || "کاربر") + '):</div>' + text.replace(/\n/g, "<br>");
            }
        } else {
            ...
            var bodyText = text;
            if (supportMatch) {
                headerHtml = '<div style="font-size:0.8rem; color:#3DD68C; font-weight:900; margin-bottom:0.4rem;">👑 پاسخ پشتیبانی:</div><div style="font-size:0.7rem; color:#8A7F72; margin-bottom:0.5rem;">📅 ' + supportMatch[1] + '</div>';
                bodyText = supportMatch[2];
            } else if (userReplyMatch) {
                headerHtml = '<div style="font-size:0.8rem; color:#E5B44E; font-weight:900; margin-bottom:0.4rem;">👤 پاسخ کاربر:</div><div style="font-size:0.7rem; color:#8A7F72; margin-bottom:0.5rem;">📅 ' + userReplyMatch[1] + '</div>';
                bodyText = userReplyMatch[2];
            } else {
                headerHtml = '<div style="font-size:0.8rem; color:#3DD68C; font-weight:900; margin-bottom:0.4rem;">👑 پاسخ پشتیبانی:</div>';
            }
  ...
  if(grid && !document.getElementById('admin-detailed-stats')){
    ...
    card.innerHTML = 
                    ...
                    '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:1rem;margin-bottom:1rem;">' +
                        '<div style="background:#171310;border:1px solid #241F18;border-radius:12px;padding:1rem;text-align:center;"><div style="font-size:0.8rem;color:#B0A695;">کل پست‌های ارسالی</div><strong style="color:#4AD6BE;font-size:1.4rem;">' + (document.querySelectorAll('#section-users tbody tr').length || 0) + ' پست</strong></div>' +
                        '<div style="background:#171310;border:1px solid #241F18;border-radius:12px;padding:1rem;text-align:center;"><div style="font-size:0.8rem;color:#B0A695;">کل کانال‌های فعال</div><strong style="color:#35C47E;font-size:1.4rem;">' + (document.querySelectorAll('#section-plans tbody tr').length || 0) + ' کانال</strong></div>' +
                        '<div style="background:#171310;border:1px solid #241F18;border-radius:12px;padding:1rem;text-align:center;"><div style="font-size:0.8rem;color:#B0A695;">تیکت‌های باز</div><strong style="color:#F98E3F;font-size:1.4rem;">' + (document.querySelectorAll('#section-tickets tbody tr').length || 0) + ' تیکت</strong></div>' +
                    '</div>' +
                    '<div style="font-size:0.8rem;color:#8A7F72;text-align:center;">آمار به صورت زنده از همین جداول محاسبه می‌شود — برای جزئیات هر کانال، تب «مدیریت کاربران» → پروفایل ۳۶۰ درجه را ببینید</div>';
