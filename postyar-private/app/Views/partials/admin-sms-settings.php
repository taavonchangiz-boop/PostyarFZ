...
<div class="card" style="margin-bottom:1.5rem;">
    ...
    <form action="<?php echo \WHCM\Core\Bootstrap::getRouteUrl('/hnnh/save-sms-config'); ?>" method="POST">
        ...
        <div class="form-group" style="max-width:520px;margin-bottom:1.25rem;">
            ...
            <select name="sms_provider_active" id="sms_provider_active" style="width:100%;padding:.75rem;border-radius:10px;background:#1E1A14;color:#fff;border:1px solid #3A3025;">
        ...
        ?>
            <section class="sms-provider-card" data-provider="<?php echo htmlspecialchars($id); ?>" style="display:none;border:1px solid #2B241B;border-radius:14px;padding:1.25rem;background:#1E1A14;margin-bottom:1rem;">
                ...
                <label style="display:flex;align-items:center;gap:.6rem;margin-bottom:1rem;cursor:pointer;">
                    <input type="checkbox" name="sms_enabled_<?php echo htmlspecialchars($id); ?>" value="1" <?php echo $enabled ? 'checked' : ''; ?> style="width:19px;height:19px;accent-color:#55C47E;">
                ...
                ?>
                    <div class="form-group">
                        ...
                        <input type="<?php echo $secret ? 'password' : 'text'; ?>" name="sms_<?php echo htmlspecialchars($id.'_'.$field); ?>" value="<?php echo $secret ? '' : htmlspecialchars($stored); ?>" placeholder="<?php echo $secret && $stored !== '' ? 'مقدار فعلی ذخیره شده است؛ برای تغییر مقدار جدید وارد کنید' : ($field === 'base_url' ? 'نشانی سرویس را در صورت نیاز وارد کنید' : ''); ?>" autocomplete="off" style="width:100%;box-sizing:border-box;padding:.65rem;border-radius:10px;background:#1E1A14;color:white;border:1px solid #3A3025;direction:ltr;text-align:left;">
                    </div>
                <?php endforeach; ?>
                </div>
                <div style="display:flex;gap:.75rem;align-items:flex-end;flex-wrap:wrap;margin-top:.5rem;">
                    ...
                    <button type="submit" name="sms_provider_active" value="<?php echo htmlspecialchars($id); ?>" formaction="<?php echo \WHCM\Core\Bootstrap::getRouteUrl('/hnnh/test-sms'); ?>" class="btn" style="min-width:170px;padding:.7rem 1.2rem;background:#5C9FE7;color:#fff;border:0;border-radius:10px;font-weight:800;">آزمون اتصال</button>
                </div>
            </section>
        <?php endforeach; ?>

        <div id="sms-test-phone" style="display:none;margin-top:1rem;border:1px dashed #3A3025;border-radius:12px;padding:1rem;">
            <label for="test_phone">شماره موبایل برای آزمون ارسال:</label>
            <input type="text" name="test_phone" id="test_phone" placeholder="۰۹۱۲۳۴۵۶۷۸۹" style="width:100%;max-width:360px;padding:.65rem;border-radius:10px;background:#1E1A14;color:#fff;border:1px solid #3A3025;direction:ltr;text-align:left;margin-top:.4rem;">
            <p style="font-size:.75rem;color:#DCD3C4;margin:.5rem 0 0;">برای پنل «اس‌ام‌اس.آی‌آر» آزمون می‌تواند یک پیام واقعی آزمایشی ارسال کند.</p>
        </div>
    </form>
</div>
            ...
            <tbody>
                ...
                <tr>
                    <td data-label="کلید رویداد">
                        <code style="background:rgba(214,172,99,0.15); color:#E9C77E; padding:0.2rem 0.5rem; border-radius:6px; font-size:0.8rem; direction:ltr; display:inline-block;"><?php echo htmlspecialchars((string)($t['event_key'] ?? '')); ?></code>
                    </td>
                    ...
                    <td data-label="عملیات">
                        <button type="button" class="btn btn-outline btn-sm" style="background:rgba(214,172,99,0.15); color:#E9C77E; border:1px solid rgba(214,172,99,0.3); padding:0.35rem 0.7rem; font-size:0.78rem; border-radius:8px; cursor:pointer;" onclick='editSmsTemplate(<?php echo json_encode($t, JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_QUOT|JSON_HEX_AMP|JSON_UNESCAPED_UNICODE); ?>)'>✏️ ویرایش</button>
        ...
        <form id="sms-template-form" action="<?php echo \WHCM\Core\Bootstrap::getRouteUrl('/hnnh/save-sms-template'); ?>" method="POST">
            ...
            <div class="form-row" style="margin-bottom:1rem;">
                <div class="form-group">
                    ...
                    <select name="event_key" id="sms_event_key" style="width:100%; padding:0.6rem; border-radius:10px; background:#1E1A14; color:white; border:1px solid #2B241B;">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="template_name" id="sms_tpl_name" placeholder="مثال: تایید پرداخت" style="width:100%; padding:0.6rem; border-radius:10px; background:#1E1A14; color:white; border:1px solid #2B241B;">
                </div>
            </div>
            <div class="form-row" style="margin-bottom:1rem;">
                <div class="form-group">
                    <label for="sms_tpl_id">شناسه قالب (TemplateId):</label>
                    <input type="number" name="template_id" id="sms_tpl_id" placeholder="مثال: 78432" min="1" style="width:100%; padding:0.6rem; border-radius:10px; background:#1E1A14; color:white; border:1px solid #2B241B; direction:ltr; text-align:left;">
                </div>
                <div class="form-group">
                    <label for="sms_tpl_params">پارامترها (JSON):</label>
                    <input type="text" name="parameters" id="sms_tpl_params" value='[]' placeholder='["name", "amount"]' style="width:100%; padding:0.6rem; border-radius:10px; background:#1E1A14; color:white; border:1px solid #2B241B; direction:ltr; text-align:left; font-family:monospace;">
                </div>
            </div>
...
<div class="card" style="margin-bottom:1.5rem;">
    ...
    <form action="<?php echo \WHCM\Core\Bootstrap::getRouteUrl('/hnnh/send-bulk-sms'); ?>" method="POST">
        ...
        <div class="form-group" style="margin-bottom:1.25rem;">
            ...
            <select name="recipient_type" id="bulk_recipient_type" onchange="toggleBulkPhoneInput()" style="width:100%; padding:0.6rem; border-radius:10px; background:#1E1A14; color:white; border:1px solid #2B241B;">
            ...
            <div style="font-size:0.78rem; color:var(--text-muted); margin-top:0.35rem;">
                تعداد کاربران فعال دارای شماره: <strong style="color:#E9C77E;"><?php echo $tf::fa_digits($recipient_count); ?></strong> نفر
            </div>
        </div>
        ...
        <div class="form-group" id="bulk_manual_phones_group" style="margin-bottom:1.25rem; display:none;">
            ...
            <textarea name="manual_phones" id="bulk_manual_phones" rows="3" placeholder="09123456789&#10;09129876543" style="width:100%; border-radius:10px; background:#1E1A14; color:white; border:1px solid #2B241B; padding:0.75rem; direction:ltr; text-align:left; font-family:monospace;"></textarea>
        </div>

        <div class="form-group" style="margin-bottom:1.25rem;">
            <label for="bulk_template_id">قالب پیامک:</label>
            <select name="bulk_template_id" id="bulk_template_id" style="width:100%; padding:0.6rem; border-radius:10px; background:#1E1A14; color:white; border:1px solid #2B241B;">
        ...
        <h3 style="font-size:0.9rem; margin-bottom:0.75rem; border-bottom:1px dashed var(--border); padding-bottom:0.4rem; color:#E9C77E;">🔧 پارامترهای قالب</h3>
        <div class="form-row" style="margin-bottom:1rem;">
            <div class="form-group">
                <label for="param1_name">نام پارامتر ۱:</label>
                <input type="text" name="param1_name" id="param1_name" placeholder="name" style="width:100%; padding:0.6rem; border-radius:10px; background:#1E1A14; color:white; border:1px solid #2B241B; direction:ltr; text-align:left;">
            </div>
            <div class="form-group">
                <label for="param1_value">مقدار پارامتر ۱:</label>
                <input type="text" name="param1_value" id="param1_value" placeholder="علی" style="width:100%; padding:0.6rem; border-radius:10px; background:#1E1A14; color:white; border:1px solid #2B241B;">
            </div>
        </div>
        <div class="form-row" style="margin-bottom:1.5rem;">
            <div class="form-group">
                <label for="param2_name">نام پارامتر ۲:</label>
                <input type="text" name="param2_name" id="param2_name" placeholder="amount" style="width:100%; padding:0.6rem; border-radius:10px; background:#1E1A14; color:white; border:1px solid #2B241B; direction:ltr; text-align:left;">
            </div>
            <div class="form-group">
                <label for="param2_value">مقدار پارامتر ۲:</label>
                <input type="text" name="param2_value" id="param2_value" placeholder="۳۰۰,۰۰۰" style="width:100%; padding:0.6rem; border-radius:10px; background:#1E1A14; color:white; border:1px solid #2B241B;">
            </div>
        </div>

        <div style="background:rgba(239,164,91,0.08); border:1px solid rgba(239,164,91,0.25); border-radius:10px; padding:0.75rem; margin-bottom:1.25rem; font-size:0.82rem; color:#F5BC82;">
...
<div class="card">
    ...
    <form method="GET" action="/index.php" style="display:flex; gap:0.75rem; margin-bottom:1.25rem; align-items:flex-end; flex-wrap:wrap;">
        ...
        <div class="form-group" style="flex:1; min-width:150px; margin-bottom:0;">
            ...
            <select name="filter_status" id="log_filter_status" style="width:100%; padding:0.5rem; border-radius:8px; background:#1E1A14; color:white; border:1px solid #2B241B;">
        ...
        <div class="form-group" style="flex:1; min-width:150px; margin-bottom:0;">
            ...
            <input type="text" name="filter_phone" id="log_filter_phone" value="<?php echo htmlspecialchars($filter_phone ?? ''); ?>" placeholder="0912..." style="width:100%; padding:0.5rem; border-radius:8px; background:#1E1A14; color:white; border:1px solid #2B241B; direction:ltr; text-align:left;">
        </div>
        ...
        <a href="/index.php?route=%2Fhnnh%2Fsms-settings" class="btn btn-outline" style="background:rgba(10,15,26,0.045); color:var(--text-muted); border:1px solid var(--border); padding:0.5rem 1rem; border-radius:8px; white-space:nowrap; text-decoration:none; margin-bottom:0;">پاک کردن فیلتر</a>
    </form>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td data-label="قالب" style="font-size:0.82rem; color:#E9C77E;"><?php echo htmlspecialchars($l['template_name'] ?? ($l['event_key'] ?? '—')); ?></td>
                        <td data-label="وضعیت">
                            <?php
                                $st = $l['status'] ?? 'pending';
                                $badge_class = 'pending';
